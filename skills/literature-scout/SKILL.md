...
description: 当用户要按领域、时间范围、排序偏好生成 Top 10 论文列表，或要快速完成某个方向的论文初筛与综述式摸底时使用；输出结构化论文清单、排序依据、图示和尽可能完整的论文/代码链接。
---
...
- 用户需要的是可执行的 Top 10 清单，而不是长篇散乱罗列。
- 用户希望把结果自动落盘为每日榜单。
...
- 如果用户没有显式传入上述参数，则读取 [configs/daily_profile.yaml](/Users/andywu/Documents/codex_workplace/projects/research-assistant/configs/daily_profile.yaml)。
- 默认配置文件现为 [configs/daily_profile.yaml](/Users/andywu/Documents/codex_workplace/projects/research-assistant/configs/daily_profile.yaml)。
- 参数优先级：
  1. 用户显式传入
  2. `configs/daily_profile.yaml`
  3. 内部兜底逻辑
...
6. 压缩为 Top 10，每篇只保留高价值信息：
   - 做了什么
   - 为什么相关
   - 为什么排在这个位置
7. 尽量补齐论文链接、官方代码、复现仓库、数据集或项目主页。
8. 能图示就图示，优先给主题树、时间线或“先读后读”阅读路径图。
9. 将输出写入 `outputs/daily_top10/YYYY-MM-DD-<field>-<ranking_profile>.md`。
...
- 输出文件必须写入：
  - `outputs/daily_top10/YYYY-MM-DD-<field>-<ranking_profile>.md`
- 如果 prompt 明确要求结构化 sidecar，则同步写：
  - `outputs/daily_top10/YYYY-MM-DD-<field>-<ranking_profile>.json`
- `YYYY-MM-DD` 使用执行当天日期。
...
- 当用户和 `configs/daily_profile.yaml` 都未指定偏好时，默认使用 `balanced-default`。
...
- 避免把同一论文的会议版、期刊版、arXiv 版重复列入 Top 10。
